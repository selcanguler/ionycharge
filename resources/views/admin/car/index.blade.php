...
@section('breadcrumbs')
<li class="breadcrumb-item active" aria-current="page">Cars Management</li>
@endsection
@section('pageTitle')
...
@endsection
@section('content')
<div class="card car_index admikoIndex">
    <div class="card-body">
        <div class="tableBox" id="tableBox">
            ...
            <div class="tableLayout pb-2">
                    ...
                    <thead>
                        <tr data-sort-method='thead'>
							...
							<th scope="col" class="text-nowrap">Marka</th>
							<th scope="col" class="text-center no-sort" data-orderable="false">Model Yönetimi</th>
                    ...
                    @foreach($tableData as $data)
                        <tr>
							<td class="w-5"><a href="{{route("admin.car.edit",[$data->id])}}">{{$data->id}}</a></td>
							<td class="text-nowrap">{{$data->brand_name}}</td>
							<td class="text-nowrap text-center childPageLink"><a href="{{route("admin.car_model.index",[$data->id])}}"><i class="fas fa-edit fa-fw"></i></a></td>
                            <td class="w-5 no-sort"><a href="{{route("admin.car.edit",[$data->id])}}"><i class="fas fa-edit fa-fw"></i></a></td>
                            @if(Gate::allows(['car_allow']))
            ...
            <div class="row">
                <div class="col-12 col-sm order-3 order-sm-0 pt-2">
                    @if(Gate::any(['car_allow']))
                        <a href="{{route('admin.car.create')}}" class="btn btn-primary" role="button"><i class="fas fa-plus fa-fw"></i> {{trans('admiko.table_add')}}</a>
                    @endIf
                </div>
    ...
    <div class="modal fade" id="deleteConfirm" tabindex="-1" role="dialog" aria-labelledby="deleteConfirm" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form method="post" class="w-100" action="{{route("admin.car.delete")}}">
            @method('DELETE')
            @csrf
