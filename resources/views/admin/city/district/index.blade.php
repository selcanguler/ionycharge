...
<li class="breadcrumb-item active" aria-current="page">İlçe Yönetimi</li>
@endsection
@section('pageTitle')
<h1>İlçe Yönetimi</h1>
@endsection
...
@endsection
@section('backBtn')
<a href="{{route("admin.city.index")}}"><i class="fas fa-angle-left"></i> {{trans('admiko.page_back_btn')}}</a>
@endsection
@section('content')
<div class="card district_index admikoIndex">
    <div class="card-body">
        <div class="tableBox" id="tableBox">
            ...
            <div class="tableLayout pb-2">
                    ...
                    <thead>
                        <tr data-sort-method='thead'>
							...
							<th scope="col" class="text-nowrap">İlçe Adı</th>
							<th scope="col" class="text-center no-sort" data-orderable="false">İstasyon Yönetimi</th>
                    ...
                    @foreach($tableData as $data)
                        <tr>
							<td class="w-5"><a href="{{route("admin.district.edit",[Request()->admiko_city_id,$data->id])}}">{{$data->id}}</a></td>
							<td class="text-nowrap">{{$data->title}}</td>
							<td class="text-nowrap text-center childPageLink"><a href="{{route("admin.station.index",[Request()->admiko_city_id,$data->id])}}"><i class="fas fa-car fa-fw"></i></a></td>
                            <td class="w-5 no-sort"><a href="{{route("admin.district.edit",[Request()->admiko_city_id,$data->id])}}"><i class="fas fa-edit fa-fw"></i></a></td>
                            @if(Gate::allows(['district_allow']))
            ...
            <div class="row">
                <div class="col-12 col-sm order-3 order-sm-0 pt-2">
                    @if(Gate::any(['district_allow']))
                        <a href="{{route('admin.district.create',[Request()->admiko_city_id])}}" class="btn btn-primary" role="button"><i class="fas fa-plus fa-fw"></i> {{trans('admiko.table_add')}}</a>
                    @endIf
                </div>
    ...
    <div class="modal fade" id="deleteConfirm" tabindex="-1" role="dialog" aria-labelledby="deleteConfirm" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form method="post" class="w-100" action="{{route("admin.district.delete",[Request()->admiko_city_id])}}">
            @method('DELETE')
            @csrf
