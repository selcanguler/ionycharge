...
@section('breadcrumbs')
<li class="breadcrumb-item active" aria-current="page">City/District/Station Management</li>
@endsection
@section('pageTitle')
<h1>City/District/Station Management</h1>
@endsection
@section('pageInfo')
@endsection
@section('backBtn')
...
@endsection
@section('content')
<div class="card city_index admikoIndex">
    <div class="card-body">
        <div class="tableBox" id="tableBox">
            ...
            <div class="tableLayout pb-2">
                    ...
                    <thead>
                        <tr data-sort-method='thead'>
							...
							<th scope="col" class="text-nowrap">İl Adı</th>
							<th scope="col" class="text-center no-sort" data-orderable="false">İlçe Yönetimi</th>
                    ...
                    @foreach($tableData as $data)
                        <tr>
							<td class="w-5"><a href="{{route("admin.city.edit",[$data->id])}}">{{$data->id}}</a></td>
							<td class="text-nowrap">{{$data->title}}</td>
							<td class="text-nowrap text-center childPageLink"><a href="{{route("admin.district.index",[$data->id])}}"><i class="fas fa-building fa-fw"></i></a></td>
                            <td class="w-5 no-sort"><a href="{{route("admin.city.edit",[$data->id])}}"><i class="fas fa-edit fa-fw"></i></a></td>
                            @if(Gate::allows(['city_allow']))
            ...
            <div class="row">
                <div class="col-12 col-sm order-3 order-sm-0 pt-2">
                    @if(Gate::any(['city_allow']))
                        <a href="{{route('admin.city.create')}}" class="btn btn-primary" role="button"><i class="fas fa-plus fa-fw"></i> {{trans('admiko.table_add')}}</a>
                    @endIf
                </div>
    ...
    <div class="modal fade" id="deleteConfirm" tabindex="-1" role="dialog" aria-labelledby="deleteConfirm" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form method="post" class="w-100" action="{{route("admin.city.delete")}}">
            @method('DELETE')
            @csrf
